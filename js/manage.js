...
$('.Listtutor').on('click', '.tutorSchedule',function () {
    $('.listClasses').html("");
    $('.tutorScheduleTitle').html($(this).attr('tutor'));
    const tutor = AV.Object.createWithoutData("_User", $(this).val());
    const query = new AV.Query('Classes');
    query.equalTo('tutor', tutor);
    query.find().then((listClasses) => {
        for(var i = 0, len = listClasses.length; i < len; i++){
            let comb = "<tr><td>" + listClasses[i].attributes.date +"</td><td>" + listClasses[i].attributes.startTime + '</td><td>' + listClasses[i].attributes.tutee + '</td></tr>';
            $('.listClasses').append(comb);
        }
    })
})
