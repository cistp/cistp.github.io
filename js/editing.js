...
$(document).ready(function () {
    ...
    query.find().then((listClasses) => {
        for(var i = 0, len = listClasses.length; i < len; i++){
            const replaceDate = listClasses[i].attributes.date.replace('-', '');
            const getTime = listClasses[i].attributes.startTime.replace(':', '');
            let comb = "<tr><td>" + listClasses[i].attributes.date +"</td><td>" + listClasses[i].attributes.startTime + '</td><td>' + listClasses[i].attributes.tutee + '</td>' + '<td><button type="button" class="removeTime btn btn-outline-primary" value="' + listClasses[i].id + '"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-trash" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/><path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4L4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/></svg></button><a name="addEvent" id="addEvent" class="btn btn-primary" href=' + `https://www.google.com/calendar/render?action=TEMPLATE&text=Tutoring%20Program&dates=${replaceDate}T${timeMapping[getTime]}Z/${replaceDate}T${timeMapping[getTime]+10000}Z&details=CIS%20Tutoring%20Program&location=Discord%20Room%20${rn}&sprop=&sprop=name:` + ' role="button">Add to Google Calendar</a></td></tr>';
